...
> **Version 5 — デリバティブ章追加版**：v4 の学部 4 年向け方針を保ちつつ、**派生証券 3 章** を追加。無裁定原理・プット-コール・パリティ → 二項モデル・リスク中立確率 → BSM・Greeks・IV スマイル・離散デルタヘッジの順に展開。Itô 解析の厳密展開は避け、二項モデルの連続極限として BSM を導く。可視化重視（payoff 図、binomial tree、Greeks、IV smile、hedge P&L 等）。
...
| [17](17_derivatives_intro.md) | デリバティブ入門：無裁定とパリティ | フォワード、コール／プット、プット-コール・パリティ |
| [18](18_binomial_model.md) | 二項モデルとリスク中立確率 | CRR、複製ポートフォリオ、BSM 連続極限 |
| [19](19_black_scholes.md) | Black–Scholes–Merton と Greeks | BSM 公式、Greeks、IV スマイル、デルタヘッジ |
| [20](20_exercises.md) | 演習問題 | 各章の補強（3 カテゴリ）|
